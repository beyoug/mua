...
export const colorMode = {
	subscribe: derived(appSettings, ($s) => coerceColorMode($s.colorMode))
		.subscribe,
...
export function resolveThemeState(
	settings: Pick<AppConfig, "theme" | "colorMode">,
	systemDark: boolean,
): {
	themeId: ThemeId;
	...
	const mode =
		colorMode === "auto" ? (systemDark ? "dark" : "light") : colorMode;
	return { themeId, mode };
}
...
export function applyThemeToDocument(state: {
	...
	if (import.meta.env.DEV) {
		...
		const missing = requiredThemeVars.filter(
			(token) => style.getPropertyValue(token).trim() === "",
		);
		if (missing.length > 0) {
			console.warn(
				`[Theme] Missing tokens for ${state.themeId}/${state.mode}:`,
				missing.join(", "),
			);
		}
	}
